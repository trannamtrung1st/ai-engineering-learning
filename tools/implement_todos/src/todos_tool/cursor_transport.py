"""Cursor Agent CLI transport: invocation, sessions, timeout, and cleanup."""
...
_active_sessions: weakref.WeakSet[CursorTransport] = weakref.WeakSet()
...
class CursorTransport:
    """Cursor CLI adapter with active-session registry and process-tree cleanup."""
    ...
    async def run_session(
        ...
        *,
        ...
        prompt_arg = (
            build_bootstrap_prompt(prompt_path, workspace)
        ...
        shell_evidence = normalizer.get_shell_commands()
...
def _collect_descendant_pids(root_pid: int) -> list[int]:
    """Collect descendant PIDs via pgrep when available."""
    try:
    ...
    all_pids: list[int] = []
    for pid in pids:
        all_pids.extend(_collect_descendant_pids(pid))
        all_pids.append(pid)
    return all_pids
