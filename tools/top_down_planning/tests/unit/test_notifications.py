...
def test_handle_audit_event_redacts_secrets_in_notification_body(bridge_send_mock) -> None:
    ...
    handle_audit_event(
        {
            "type": "run_failed",
            "reason": """password=hunter2-password api_key=sk-example-key accessToken=camel-notify-secret curl -d '{"password":"json-pass-secret"}'""",
        },
        run_id="run-1",
        options=options,
        phase="planning",
    ...
    assert "camel-notify-secret" not in combined
