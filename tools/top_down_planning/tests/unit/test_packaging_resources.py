...
def test_built_wheel_includes_production_prompt_templates(tmp_path: Path) -> None:
...
def test_built_wheel_excludes_test_prompt_templates(tmp_path: Path) -> None:
    wheel = _build_wheel(tmp_path / "dist")
...
def test_installed_wheel_renders_planner_protocol(tmp_path: Path) -> None:
    wheel = _build_wheel(tmp_path / "dist")
    site = tmp_path / "site"
    site.mkdir()
    python = _pip_python()
