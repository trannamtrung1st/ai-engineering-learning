...
def _create_planning_run(store: FileRunStore, run_id: str = "run-cap") -> None:
...
def test_capability_record_stores_hash_not_secret(tmp_path: Path) -> None:
    ...
    token = grant_capability(store, "run-cap", role="planner", phase=PLANNING)
    token_id = token.split(".", 1)[0]
    record = store.load_capability("run-cap", token_id)
    assert "secret_hash" in record
    assert "secret" not in record
    path = store.capabilities_dir("run-cap") / f"{token_id}.json"
...
def test_wrong_session_is_denied(tmp_path: Path) -> None:
    ...
    store.save_run("run-cap", run, expected)

    with pytest.raises(CapabilityDeniedError, match="session"):
        authorize_mutation(store, "run-cap", operation="plan_apply", capability_token=token)
...
def test_revoked_token_is_denied(tmp_path: Path) -> None:
    ...
    token = grant_capability(store, "run-cap", role="planner", phase=PLANNING)
    token_id = token.split(".", 1)[0]
    store.revoke_capability("run-cap", token_id)
...
def test_phase_leave_revokes_capabilities(tmp_path: Path) -> None:
    ...
    _create_planning_run(store)
    token = grant_capability(store, "run-cap", role="planner", phase=PLANNING)
    revoke_capabilities_for_phase(store, "run-cap", PLANNING)
    with pytest.raises(CapabilityDeniedError, match="revoked"):
        authorize_mutation(store, "run-cap", operation="plan_apply", capability_token=token)
...
def test_planner_cannot_use_reviewer_authority_from_disk(tmp_path: Path) -> None:
    ...
    for path in store.capabilities_dir("run-cap").glob("*.json"):
        payload = json.loads(path.read_text(encoding="utf-8"))
        assert payload.get("role") != "reviewer" or "secret" not in payload

    service = ReviewAgentService(store, "run-cap")
    ...
    store.save_review(
        "run-cap",
        {
            **store.load_review("run-cap", "review-whole-plan-01"),
            "reviewer_session_id": "reviewer-session-01",
        },
    )
    with pytest.raises(CapabilityDeniedError, match="session"):
