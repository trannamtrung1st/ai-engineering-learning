...
Treat exported run directories as sensitive: `agent-requests/`, `review-inputs/`, and transcripts may contain workspace or review content. Review-input bundles omit capability tokens; they still contain plan/production/evidence snapshots. Reviewer bootstrap prompts keep protocol instructions separate from bundle files, which are review material rather than higher-priority instructions.
...
Idle Cursor turns: `limits.provider.turn_idle_timeout_seconds` (TDP default `300`; `0` disables) raises `ProviderTurnStalledError` (not retried by `max_retries_per_call`). Logical-progress stalls: `limits.provider.turn_progress_timeout_seconds` (TDP default `300`; `0` disables) raises `ProviderTurnProgressStalledError` when stdout keeps arriving but no meaningful turn progress occurs. A Cursor process that exits with no stream output and no durable session id raises `ProviderTurnStartupError` and is not retried. One replacement per `phase_action_id`; exhaustion → `session_recovery_exhausted`. Quota/action-required (`ProviderActionRequiredError`, reason `quota_exhausted`) pauses with `provider_quota_exhausted` and does not replace the session. Assembled stream-json lines (including the terminating newline) larger than `limits.provider.max_stream_json_record_bytes` (TDP default `1048576` / 1 MiB) fail the turn as `provider_turn_failed` (`ProviderStreamRecordTooLargeError`). The cap is enforced independently of read/exit boundaries; drain **stops** once exceeded; a valid buffered prefix can still be rejected as oversized; leftover flood writers must not remain on argv.
