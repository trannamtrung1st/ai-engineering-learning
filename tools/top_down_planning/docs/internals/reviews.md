...
Successful focused review persists `status=approved` (terminal) and `focused_review_approved` in one commit. Historical `production.blocker_report` values that wait on that loop are resolved only when the same loop and target revision match, and a persisted `target_digest` requires the satisfying review to persist the same digest. Untyped legacy blockers are rebound from event history only when that history uniquely proves a focused-review wait; otherwise `tdp resume --check` reports `ambiguous_legacy_blocker` and does not auto-clear. External blockers stay active. An active review-bound wait pauses production (`status=paused`, `stop.code=focused_review_wait`, `outcome=null`) instead of completing `outcome=blocked`. `tdp resume --check` reports stale or unsatisfiable review-bound blockers without mutating them.
