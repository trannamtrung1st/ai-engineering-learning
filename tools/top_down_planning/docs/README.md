...
Role `protocol_instructions` in planner, producer, and reviewer manifests are rendered Markdown strings from package-owned Jinja templates under `src/top_down_planning/prompts/templates/`. Edit those templates (and `prompts/contexts.py` context flags) rather than inline Python strings in orchestrator session modules. Templates ship in the installed package (editable monorepo install and CI wheel verification); `jinja2` is a runtime dependency.
