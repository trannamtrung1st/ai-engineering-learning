...
Agent session lifecycle lines use `[session:start]`, `[session:resume]`, and `[session:end]`. Audit events `planner_session_started`, `producer_session_started`, and `reviewer_session_started` are emitted after the orchestrator allocates a provider session and before the first provider turn is consumed (`role`, `phase`, `run_id`, and `session_id` required). Matching `*_session_resumed` events are emitted before each `resume_primary_session` call and when resuming an in-flight reviewer turn. After each blocking phase step and before provider teardown (including Ctrl+C cancel), `RunEngine` emits one `[session:end]` line per active provider session with the same fields. Run-level CLI messages use `[run:start]` and `[run:resume]`; persisted `run_created` audit events map to `[run:start]`.
