...
| `focused_review_wait` | Production paused while a review-bound blocker is still active. Resume drives any pending focused review, then re-evaluates the blocker and continues if it is satisfied. `tdp resume --check` reports stale or unsatisfiable review-bound blockers without mutating them. |
...
| Provider unavailable / turn stalled | Cursor missing from PATH, idle timeout (`limits.provider.turn_idle_timeout_seconds`, default `300`, `0` disables), logical-progress timeout (`limits.provider.turn_progress_timeout_seconds`, default `300`, `0` disables), silent zero-output startup (`ProviderTurnStartupError`), or remote session gone. One replacement per `phase_action_id`; exhausted replacement fails with `session_recovery_exhausted` (not resumable as a normal pause). Inspect `review-inputs/<loop-id>/<attempt-id>/manifest.json` when a reviewer turn failed after session start. |
| `provider_quota_exhausted` | Cursor reported account/provider quota exhaustion (for example `You're out of usage`). The run pauses operationally; workspace changes and the interrupted `phase_action_id` are preserved. This does **not** replace the provider session. Restore capacity, then `tdp resume --run <id>`. |
