...
| Provider unavailable / turn stalled | Cursor missing from PATH, idle timeout (`limits.provider.turn_idle_timeout_seconds`, default `300`, `0` disables), or remote session gone. One replacement per `phase_action_id`; exhausted replacement fails with `session_recovery_exhausted` (not resumable as a normal pause). |
