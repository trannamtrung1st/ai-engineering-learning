...
Missing remote sessions (`provider_session_not_found`) and idle/progress stalls (`ProviderTurnStalledError` / `ProviderTurnProgressStalledError` when `limits.provider.turn_idle_timeout_seconds` or `limits.provider.turn_progress_timeout_seconds` > 0; TDP default `300`, `0` disables that watchdog) each allow **one** replacement per `phase_action_id` with a recovery manifest. Lineage reasons: `provider_session_not_found` or `provider_turn_stalled`. Exhausted replacement fails the run (`session_recovery_exhausted`). Quota/action-required failures pause with `provider_quota_exhausted` and do **not** consume that replacement budget. An oversized assembled stream-json line including the terminating newline (`limits.provider.max_stream_json_record_bytes`, TDP default `1048576` / 1 MiB) fails the turn as `provider_turn_failed` and does not consume that replacement budget.
