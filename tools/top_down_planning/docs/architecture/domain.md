...
Structured `blocker_report` values are either `external` (genuine terminal blocked outcome) or `focused_review_wait` (recoverable pause). Before terminalizing, orchestration re-evaluates review-bound identity (loop, revision, and digest when the blocker recorded one). Untyped legacy blockers are rebound only from temporally causal event history (`request` while unresolved, then `blocked reported`, then `approved`, including same-loop recheck replay). An active wait pauses with `stop.code=focused_review_wait` rather than `outcome=blocked`. The stale-blocker / focused-review recovery cluster is closed; details: [review architecture](../internals/reviews.md).
