...
Whole-plan review (proposal §5.2, §11): the orchestrator allocates a fresh reviewer session id, binds a capability token, delivers the bounded review package, and consumes the review turn. Mandatory gates run two stages: `initial_review` (discovery), optional `finding_verification` (close known findings and direct side effects after revisions), then a fresh `scope_blocker_review` (search remaining approval blockers without prior-finding framing). Clear initial approval still requires a separate blocker stage — finding closure alone is not final approval. Stage responds require `stage` plus Result Contract fields (`tdp agent example review-respond-initial`, `review-respond-verification`, `review-respond-blocker`). Each loop binds findings to the current plan revision, resumes the same primary planner for revisions after `changes_requested`, and requires verification recheck (`finding_verification` delivery) before blocker review when findings were raised. Review packages include an optional `rubric` on initial review only; blocker review omits the full quality rubric. After the gate completes, deterministic `validate_plan(..., mode="approval")` must pass before the run advances to `plan_validated`. Revision cycles are capped per finding set by `limits.whole_plan_review.max_revision_cycles`; blocker rounds use `max_blocker_review_rounds`. Limit exhaustion yields `limit_reached` / `rejected`, never silent acceptance.
