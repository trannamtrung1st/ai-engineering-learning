...
Direct unit execution (`tdp execute --unit`) with dependencies requires an explicit complete `--upstream dep=<child-run-id>` map; each upstream run must belong to the mapped dependency unit and pass accepted-delivery validation. Child creation authorizes configured **resource** snapshot drift from the cumulative workspace baseline (direct upstream results plus previously accepted sibling/closure results in the same package), not only one-hop dependencies; guidance and skill drift are always rejected. Parent resume similarly authorizes attached accepted-child evidence before synthesis. Child package bindings are immutable after execution starts (retrofit only while `plan_validated` with no batches/sessions). Accepted-child delivery validation recomputes the live production output digest and requires `completed` / `output_validated` / `accepted`. Terminal children are revalidated before reuse. Package IDs are validated as store IDs and confined under `.execution_packages/`.

Child runs bind direct semantic dependencies as digest-verified wrappers (`accepted_result`, `accepted_result_digest`, `upstream_contract_digest`) and persist `workspace_baseline_accepted_results` for context authorization. Context authorization paths come only from persisted `accepted_result.output_refs` — live production is never reloaded as an authorization fallback. Accepted-result attestations extract delivery from live production batches at record time (`output_evidence`, `batches[].result.outputs`, `batches[].result.contributions`) and require the live output digest to match `run.digests.output`.
