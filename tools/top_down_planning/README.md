...
Planner, producer, and reviewer packages include `protocol_instructions` (a rendered Markdown string from package-owned Jinja templates) and `tool_instructions` (`tdp agent` command templates). The provider adapter surfaces `protocol_instructions` at the top of the prompt so agents do not substitute host IDE planning artifacts for persisted `tdp agent` mutations.
