...
| Module | Contents |
| --- | --- |
| `core_tools.provider` | Provider protocol, stub/Cursor adapters, stream normalization (`text` or `message.content`), tool-call lifecycle events (`subtype: started` and `completed`; `tool_result` dropped), session references, `list_active_sessions()` (sessions currently retained in the in-memory registry; `session_id`, `role`, `kind`, `model`), subprocess cleanup. Normalized stream events carry `session_id`; model labels live on session references and lifecycle audit events only. Session payloads use `format_manifest_prompt` / `format_request_prompt`, surfacing optional `protocol_instructions` (Markdown string) before the JSON body. Cursor non-interactive argv uses `--print --output-format stream-json --trust --approve-mcps --force` so shell/`tdp agent` tool calls are not rejected. Transient in-memory `cursor-pending-*` handles are never passed to Cursor `--resume`. A completed Cursor process with zero usable stream records and no durable session id raises `ProviderTurnStartupError` and is not retried by `max_retries_per_call`; retry/error diagnostics include prompt/argv byte counts without prompt text. Cursor turns run on background collector threads; `abort_turn()` clears buffered stream events, stops the tracked subprocess, wakes `stream_events` waiters, and keeps the durable session for follow-up turns; callers that must block until the collector finishes pair `abort_turn()` with `wait_turn_settled()`; `wait_turn_settled()` also blocks before `_queue_turn` accepts the next prompt; `terminate_all_sessions()` kills every tracked agent subprocess pid (SIGTERM grace period, then SIGKILL) and returns termination records for orchestrator audit events. The default process runner stops reading stdout once the child exits so exited/zombie subprocesses cannot block turn drain indefinitely. `limits.provider.turn_idle_timeout_seconds` (adapter fallback `300`; `0` disables) ends a turn when no stream-json stdout arrives within the interval and raises `ProviderTurnStalledError` (not retried by `max_retries_per_call`). `limits.provider.turn_progress_timeout_seconds` (adapter fallback `300`; `0` disables) ends a turn when no meaningful progress arrives (assistant/thinking text, tool_call started/completed, terminal result/error) and raises `ProviderTurnProgressStalledError`. Idle/progress watchdogs decide when a turn must stop; collector and process-tree teardown use a separate monotonic cleanup budget (`DEFAULT_TURN_TREE_CLEANUP_SECONDS`, currently `2`) that is not derived from watchdog settings. A non-cooperative custom iterator that ignores `close()` may leave a daemon `cursor-idle-stream` thread alive after cleanup budget exhaustion; the adapter fails closed with `ProviderTurnCleanupError` (or annotates the primary turn error) rather than reporting successful teardown. Quota/action-required wording such as `You're out of usage` raises `ProviderActionRequiredError`. `limits.provider.max_stream_json_record_bytes` caps one assembled stream-json line including the terminating newline (`ProviderStreamRecordTooLargeError`); the adapter fallback is 256 KiB when the key is omitted or invalid/non-positive, and TDP's resolved default is 1 MiB (TDP config requires an integer >= 1). `CursorProvider` construction and process-tree termination are POSIX-only; Windows raises `ProviderUnsupportedPlatformError`. |
...
| `core_tools.observability` | Structured `ConsoleEvent` model, `EventSink` protocol, redaction (`RedactionPolicy` with opt-in `max_message_length`; unlimited by default; capability tokens, secret-bearing keys, authorization headers, and sensitive assignments), `flush_stream()` to close a thinking/response block, colorized stderr console renderer (discrete category blocks vs incremental `thinking`/`response` deltas; explicit `\n` in agent text breaks lines within a block), JSONL sinks (thinking/response aggregated per block, split on category/session/`flush_stream`), agent text delta streaming (`AgentTextStreamController`) |
