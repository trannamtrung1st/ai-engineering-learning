...
| `core_tools.observability` | Structured `ConsoleEvent` model, `EventSink` protocol, redaction, colorized stderr console renderer (category blocks: prefix on the first line only; all lines in a block share category styling), JSONL sinks, agent text sentence streaming (`AgentTextStreamController`) |
