...
| **Navigation** | Forbidden links in chrome, dead-end pages, broken back | Participant sees admin route in sidebar |
...
| Field | Rule |
| --- | --- |
| **ID** | `UX-<slice-id>-NNN` — e.g. `UX-web-participant-browse-003`; increment per slice per run |
...
| **P1** | Core flow blocked (cannot submit, cannot navigate to required screen); forbidden link visible to wrong role |
...
4. Do **not** duplicate: if a `TC-*: FAIL` already covers the issue, skip a separate UX entry
