...
| Skills | `skills/*/SKILL.md` — agent craft guidance (`frontend-design`, `design-craft-notion`, `ui-ux-testing`); wired via `context-map.json` `alwaysRead`, injected into prompts by `build-prompt.sh` |
...
**Product customization:** replace `verify-scenarios.sh` with an app-specific live-stack probe when the default scenario probe does not apply (e.g. auth/login envelope instead of participant registration).
