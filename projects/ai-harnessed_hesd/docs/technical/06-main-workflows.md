...
Preview uses natural world orientation per NFR-14 — rear camera unmirrored; user/unknown cameras mirrored consistently in preview and decode ([12-backend-frontend-tech-stack.md](./12-backend-frontend-tech-stack.md) §4.4.1a). Lecturer display renders server-issued `qrPayload` only; token minting stays server-side.
