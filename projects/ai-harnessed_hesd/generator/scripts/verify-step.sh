...
run_validator() {
  ...
  case "$vid" in
    ...
    actors-valid)
      meta="$(product_meta_file)"
      if [[ ! -f "$meta" ]]; then
        gen_err "product-meta.json missing — cannot validate actors"
        fail=1
      else
        pa="$(jq -r '.actors[0] // empty' "$meta")"
        sa="$(jq -r '.actors[1] // empty' "$meta")"
        if [[ -z "$pa" ]]; then
          gen_err "actors[0] (primary actor) missing or empty in product-meta.json"
          fail=1
        elif [[ -z "$sa" ]]; then
          gen_err "actors[1] (secondary actor) missing or empty in product-meta.json"
          fail=1
        else
          gen_ok "actors-valid ok (${pa}, ${sa})"
        fi
      fi
      ;;
    outputs-exist)
    ...
    harness-customize)
      ...
      unresolved=0
      for ph in '{{PRODUCT_NAME}}' '{{BRANCH_PREFIX}}' '{{WORKSPACE_NAME}}' '{{PRODUCT_SLUG}}' '{{PRIMARY_ACTOR}}' '{{SECONDARY_ACTOR}}'; do
    ...
    placeholder-scan-harness)
      ...
      gen_placeholders=('{{PRODUCT_NAME}}' '{{BRANCH_PREFIX}}' '{{WORKSPACE_NAME}}' '{{PRODUCT_SLUG}}' '{{PRIMARY_ACTOR}}' '{{SECONDARY_ACTOR}}')
      while IFS= read -r out; do
        [[ -z "$out" ]] && continue
        abs="$(resolve_repo_path "$out")"
        [[ -f "$abs" ]] || continue
