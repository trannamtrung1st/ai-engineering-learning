...
> {{DESIGN_STYLE_NAME}} uses {{SHADOW_STYLE}} shadows with no blur. The shadow color matches the border token (black in light mode, dark gray in dark mode).
...
- All shadows are {{SHADOW_STYLE}} (no blur radius, no spread except xl/2xl)
