...
validate_min_per_category() {
  local cat min_count count policy_json
  policy_json="$(match_category_policy_for_tag)"
  for cat in functional non-functional edge; do
    min_count="$(echo "$policy_json" | jq -r --arg c "$cat" '.minCasesPerCategory[$c] // empty' 2>/dev/null || true)"
    if [[ -z "$min_count" ]]; then
