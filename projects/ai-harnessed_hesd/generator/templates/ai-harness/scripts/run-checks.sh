      ...
      if [[ "$script" == "test:playwright-ui" && -n "$SLICE_ID" && "$(playwright_scope_for_checks)" == "slice" ]]; then
        ...
        if script_needs_test_stack "$script"; then
          if ! prepare_test_stack_for_script "$script"; then
            aih_check_begin "${label} (test stack reset)"
            aih_check_fail "test stack reset failed before ${script}"
            FAILURES+=("{\"type\":\"runtime\",\"message\":\"test stack reset failed before ${script}\"}")
            PASS=false
            set -e
            continue
          fi
...
check_test_stack_runtime() {
  local label failed=false
  ...
  if ! docker compose \
    ...
    config -q >/dev/null 2>&1; then
    FAILURES+=("{\"type\":\"runtime\",\"message\":\"test stack compose config invalid ($(test_compose_file))\"}")
    PASS=false
    failed=true
    aih_err "test stack compose config invalid"
  fi
...
check_db_runtime() {
  local profile failed=false
  profile="$(get_check_profile)"

  if ! command -v docker >/dev/null 2>&1; then
    if check_profile_needs_test_stack "$profile"; then
      aih_check_begin "db runtime validation"
      FAILURES+=("{\"type\":\"runtime\",\"message\":\"docker not available for db validation\"}")
      PASS=false
      aih_check_fail "docker not available"
    else
      aih_check_skip "db runtime validation (docker not available; profile ${profile} has no integration/e2e)"
    fi
    return
  fi
  ...
  if [[ -f package-lock.json ]] && file_contains 'better-sqlite3|sqlite3' package-lock.json; then
    ...
    PASS=false
    aih_check_fail "SQLite dependency found in package-lock.json"
    return
  fi

  if ! check_profile_needs_test_stack "$profile"; then
    aih_check_skip "db runtime validation (profile ${profile} has no integration/e2e)"
    return 0
  fi
...
if [[ ${#FAILURES[@]} -gt 0 ]]; then
  failures_json="$(attach_log_excerpts_to_failures_json "$failures_json" "$SLICE_ID" 8000)"
fi
