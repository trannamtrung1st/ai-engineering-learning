...
Harness-owned skills live under [`skills/`](skills/) (e.g. [`skills/visual-design/SKILL.md`](skills/visual-design/SKILL.md)). They are **not** auto-discovered by Cursor IDE — Ralph injects them via `config/context-map.json` → `build-prompt.sh` → agent prompts.

| Skill | Agents | Purpose |
|---|---|---|
| `visual-design` | `frontend` implementer, browser `tester` | Design-system module reading, style profile craft, screenshot self-critique |
...
| `AIH_GENERATED_RETENTION_MINUTES` | `60` | TTL for `ai-harness/generated/` run artifacts, screenshots, and evidence (`0` disables cleanup) |
| `AIH_SKIP_GENERATED_CLEANUP` | — | Skip generated artifact TTL cleanup (`1`) |
...
```bash
./ai-harness/scripts/run-ai-review.sh api-foundation
```
...
npm run aih:testgen:enhance -- FR-08 "Add browser-journey cases for listing pagination and sort"
...
### Integration/e2e failure triage

When `test:integration`, `test:e2e`, or cross-slice failures block a slice:
...
- `ai-harness/docs/test-failure-triage.md` — integration/e2e flake triage runbook
