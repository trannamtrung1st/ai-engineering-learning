...
2. **Home link always visible** — `SidebarNav` must include the role's primary dashboard link at the top of the list (e.g. "Phiên học" → `/lecturer/sessions` for Lecturer, "Thiết lập học kỳ" → `/admin/terms` for AcademicAdmin). The **Attendly logo in the sidebar or mobile header is the primary home shortcut** — it must be a clickable link to the role's home route, not decorative-only. `StudentLayout` must include a "Trang chủ" link or linked Attendly logo pointing to `/check-in`.
