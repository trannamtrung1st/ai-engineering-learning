...
Canonical harness spec for starting and verifying the Attendly stack (database, API, web). Referenced by `ai-harness/README.md`, `ai-harness/HARNESS-DESIGN.md`, and `docs/technical/13-docker-compose-local-runtime.md`.
...
4. `npm run build --workspace @attendly/api` — API `dev` script runs `node --watch dist/index.js`.
...
```bash
#!/usr/bin/env bash
# Start Attendly stack (dev or full preview) and verify API + web startup.
...
if [[ "$MODE" == "full" ]]; then
  ...
  npm run build --workspace @attendly/api
  stop_dev_processes
  : > "$PID_FILE"
  PORT="${AIH_PREVIEW_API_PORT:-3001}" npm run dev --workspace @attendly/api >>"${RUNS_DIR}/preview-api.log" 2>&1 &
  echo $! >> "$PID_FILE"
  PORT="$(aih_web_port)" npm run dev --workspace @attendly/web >>"${RUNS_DIR}/preview-web.log" 2>&1 &
  echo $! >> "$PID_FILE"
fi
...
## Preview vs test stack isolation
...
```bash
npm run aih:test:stack:reset && npm run test:integration
```

Playwright `globalSetup` refresh (when present) is still useful for preview fixture idempotency, but integration tests no longer pollute preview DB when the test stack is configured.
