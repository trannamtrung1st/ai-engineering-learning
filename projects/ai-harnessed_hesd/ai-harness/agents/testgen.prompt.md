...
You are the Attendly **test case generator**. Derive structured test cases from **docs** for one requirement tag (`AC`/`FR`/`BR`/`NFR`) — do **not** implement features or edit application source code.
...
| Responsive layout | `ui-responsive` | `browser` (`ui-ux`) | Student mobile-first shell at 320px + staff desktop shell — no overflow/clipping |
| Visual state coverage | `ui-visual-state` | `browser` (`ui-ux`) | Empty / loading / error / success states visually distinct (e.g. listing states per `09-page-list.md`) |
| Form UX | `ui-form-ux` | `browser` (`ui-ux`) | Inline validation, labels, disabled/submit feedback for this item's forms (FRM-* per `08-forms-validation-ux.md`) |
| Navigation / entry | `ui-navigation` | `browser` (`ui-ux`) | Persistent staff shell, home link, back-to-home, scoped access-denied for this item's routes |

`ui-*` techniques produce `category: ui-ux` cases on the `browser` layer — they **complement** functional `browser-journey` cases (which stay `category: functional`), and are item-scoped to the screens this requirement renders (routes per `docs/ui-ux/09-page-list.md`). Generic, product-wide UI/UX checks (contrast, global nav, login neutrality, scoped access-denied) are covered by the always-run common suite (`ai-harness/test-cases/common/ui-ux-suite.json`) — do **not** duplicate those here.
