...
> **Implementation status (2026-07-07):** Repository `docker-compose.yml` provides **`db` (Postgres) only** on port 5432 with `pg_isready` health checks, the named `attendly-postgres-data` volume, and the `attendly-local-net` bridge network. `docker-compose.test.yml` provides an isolated ephemeral Postgres on port 5433 for integration/e2e. Reset the dev DB with `npm run db:reset` (soft) or `npm run db:reset:hard` (drop volume) via [`scripts/db-reset.sh`](../../scripts/db-reset.sh); migrate/seed hooks run only once the backend defines `db:migrate`/`db:seed`. Target topology below (Redis, api, web containers, `full-preview` profile) is tracked in harness slice `compose-full-preview-redis`. Dev preview uses local API/web processes via `npm run aih:preview` — see [14-integration-debt.md](./14-integration-debt.md).
