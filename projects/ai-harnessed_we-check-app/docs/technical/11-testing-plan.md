...
| Playwright UI regression | ~5% | `tests/playwright-ui` — `@playwright/test` | Committed browser specs generated by harness browser tester |
| Browser / device | Ad hoc + gate | Playwright MCP; physical devices for GPS/camera | Mobile check-in, QR scan, projector QR readability |
...
| Playwright UI | `npm run test:playwright-ui` | Preview web at `localhost:3007` (optional until workspace slice passes) |
| Build | `npm run build` | — |
...
**Browser gate (frontend slices):** Playwright MCP verifies `layer: browser` cases from TestGen artifacts; browser tester also performs UX audit (`UX-*` bugs) and writes committed Playwright regression specs to `tests/playwright-ui/scenarios/`. Playwright config sets `VITE_ENABLE_DEVICE_SIMULATION=true` for E2E ([NFR-24](../brds/07-non-functional-risk.md)). Mobile device matrix on ≥ **4** devices before pilot ([NFR-18](../brds/07-non-functional-risk.md)).
...
| Playwright regression index | `ai-harness/playwright-regression-index.json` |
| Playwright UI specs | `tests/playwright-ui/scenarios/<slice-id>.spec.ts` |
