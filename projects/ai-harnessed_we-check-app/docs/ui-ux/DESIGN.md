...
# We Check — Application Scope

This file is the **authoritative design spec** for We Check, vendored from [VoltAgent awesome-design-md `notion/DESIGN.md`](https://github.com/VoltAgent/awesome-design-md/blob/main/design-md/notion/DESIGN.md). CSS implementation mapping lives in [04-design-tokens.md](./04-design-tokens.md). Visual direction summary: [01-design-overview.md](./01-design-overview.md) §5.

**Supersedes:** Campus Pulse v2 (indigo brand, blue CTAs, Plus Jakarta + Be Vietnam Pro).

## In scope for We Check

| Area | DESIGN.md components / tokens |
| --- | --- |
| Workspace UI | `button-*`, `text-input`, `search-pill`, `card-base`, `pill-tab-*`, `badge-tag-*`, surfaces, hairlines, table density |
| Auth (desktop) | `hero-band-dark`, `button-primary`, `button-secondary-on-dark` on split login panel |
| Attendance semantics | `semantic-success` / `semantic-warning` / `semantic-error`, `card-tint-*` washes for status badges and check-in outcomes |
| Typography | Notion Sans → **Inter** (public equivalent with Vietnamese subset) per [04-design-tokens.md](./04-design-tokens.md) §4 |

## Out of scope (marketing surfaces)

Do **not** implement in We Check app routes: pricing tiers (`pricing-card-*`), FAQ accordions, logo walls, sticky-note hero decorations, pastel marketing feature bands, testimonial cards, comparison tables, `workspace-mockup-card` hero embeds.

## We Check domain overrides (non-negotiable)

| Override | Spec |
| --- | --- |
| QR presentation | Max-contrast black/white — [NFR-20](../brds/07-non-functional-risk.md); tokens in [04-design-tokens.md](./04-design-tokens.md) §3.5 |
| Touch targets | ≥ **44×44 px** on student routes — [00-production-ui-quality-bar.md](./00-production-ui-quality-bar.md) |
| Locale | Vietnamese (`vi-VN`) UI copy |
| Attendance status | Token mapping in [04-design-tokens.md](./04-design-tokens.md) §12–13 |

**Upstream sync:** Re-vendor from the GitHub URL above when updating; preserve this preamble block.
