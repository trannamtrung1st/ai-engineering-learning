...
| Description | Email/username and password authentication. Redirects authenticated users to role home. Preserves `returnUrl` for check-in deep links. Footer includes optional *Quay về trang chủ* → `/` ([FR-18](../brds/03-functional-requirements.md)). |
...
| Description | Unknown routes; recovery CTA to role home when authenticated or `/login` when unauthenticated ([FR-18](../brds/03-functional-requirements.md)). |

### 2.4 Root / Shell overview

| Attribute | Value |
| --- | --- |
| Route | `/` |
| Layout | `RootLayout` (no role nav) |
| Role | Unauthenticated visitors only (authenticated users redirect per AC-18e) |
| Priority | Must |
| Primary components | `ShellOverviewPage`, `RouteDiscoveryPanel`, `CheckInOutcomeShowcase`, `QrCountdown` |
| Description | Component showcase for design-system review plus **route discovery** quick links to `/login`, `/check-in`, `/sessions`, `/admin`. Discovery panel hidden when authenticated ([AC-18g](../brds/08-acceptance-mvp-future.md)). |

**Traceability:** FR-18 · AC-18f · AC-18g · NFR-17
...
| Role | `Student` |
| Priority | Must |
| Primary components | `AttendanceHistoryList`, `StatusBadge`, `EmptyState`, `TableToolbar` |
| Description | Read-only paginated list of student's own attendance across enrolled subjects. |
| Listing compliance | [14-listing §0](./14-listing-pages-search-filter-sort.md) — search (subject), status filter, sort, load-more pagination |
...
| Role | `Instructor` |
| Priority | Must |
| Primary components | `PageHeader`, `SessionCard`, `Button` (create), `EmptyState`, `TableToolbar` |
| Description | Lists instructor's sessions with search, status filter chips, sort, and pagination. **Tạo buổi học** navigates to create form. |
| Listing compliance | [14-listing §0](./14-listing-pages-search-filter-sort.md) — search (class/subject), status chips, sort, offset/load-more |
...
| Components | `SessionMonitorDashboard`, `StatCard`, `SpoofAlertBadge`, `TableToolbar` |
...
| Listing compliance | [14-listing §0](./14-listing-pages-search-filter-sort.md) — student search, status filter, sort |
...
| Components | `AttendanceRosterTable`, `AttendanceEditDialog`, `AttendanceAuditTrail`, `TableToolbar` |
| Description | Full roster with manual edit actions during and up to 24 h after close. |
| Listing compliance | [14-listing §0](./14-listing-pages-search-filter-sort.md) — student search, status filter, column sort |
...
| Role | `Instructor` |
| Priority | Must |
| Primary components | `ReportFilterBar`, `ReportSummaryCards`, `SessionReportTable`, `AttendanceRosterTable`, `Button` (export) |
...
| Listing compliance | [14-listing §0](./14-listing-pages-search-filter-sort.md) — `ReportFilterBar`, table sort, pagination |
| Export | Inline **Xuất CSV** on report pages; scoped to assigned class-subject ([AC-13c](../brds/08-acceptance-mvp-future.md), [BR-09](../brds/04-business-rules.md)) |
...
Shell: `AdminLayout` · Sidebar: **Trang chủ**, Người dùng, **Lớp học**, **Môn học**, Danh sách ghi danh, Nhập danh sách, Báo cáo, Xuất CSV, Chính sách. Nav items omitted when permission missing ([FR-18](../brds/03-functional-requirements.md), [BR-14](../brds/04-business-rules.md)).
...
| Description | Post-login admin landing. Permission-filtered cards: **Quản lý người dùng**, **Lớp học**, **Môn học**, **Danh sách ghi danh**, **Nhập CSV**, **Báo cáo**, **Xuất CSV**, **Chính sách** — each hidden if permission missing. |
...
| Description | Searchable directory of all users. **Thêm người dùng** → create form; **Nhập CSV** → bulk user import. |
| Listing compliance | [14-listing §0](./14-listing-pages-search-filter-sort.md) — search, role/status filters, sort, pagination |
...
| Role | `TrainingOfficeAdmin` |
| Priority | Must |
| Primary components | `UserImportForm`, `FileUpload`, `ImportPreviewTable`, `ImportSummary` |
| Description | Bulk CSV upload with validation preview and import summary. Upsert by `institutional_id` for large cohorts (1000+ rows). |

**Traceability:** FR-01 · AC-01d, AC-01e, AC-01f, AC-01g

### 5.3 Class Rosters (enrollments)
...
| Description | View student enrollments per class. Link to import flow. Distinct from class catalog at `/admin/classes`. |
| Listing compliance | [14-listing §0](./14-listing-pages-search-filter-sort.md) — class search, cohort filter, sort, pagination on detail |
...
| Role | `TrainingOfficeAdmin` |
| Priority | Must |
| Primary components | `PageHeader`, `ClassCatalogTable`, `TableToolbar`, `Button` (**Thêm lớp**) |
| Description | List of `Class` reference records. **Thêm lớp** → `/admin/classes/new`. |
| Listing compliance | [14-listing §0](./14-listing-pages-search-filter-sort.md) — code/name search, sort, pagination |

**Traceability:** FR-03 · AC-03d · AC-03e · AC-03i
...
| Role | `TrainingOfficeAdmin` |
| Priority | Must |
| Primary components | `ClassForm`, `FormActions` |
| Description | Create a single `Class` reference record. Uppercase code validation; duplicate code shows localized error. Success → toast + redirect `/admin/classes`. |

**Traceability:** FR-03 · AC-03d · AC-03e

### 5.3c Subject Catalog
...
| Role | `TrainingOfficeAdmin` |
| Priority | Must |
| Primary components | `PageHeader`, `SubjectCatalogTable`, `TableToolbar`, `Button` (**Thêm môn**) |
| Description | List of `Subject` reference records. **Thêm môn** → `/admin/subjects/new`. |
| Listing compliance | [14-listing §0](./14-listing-pages-search-filter-sort.md) — code/name search, sort, pagination |

**Traceability:** FR-03 · AC-03f · AC-03g · AC-03i
...
| Role | `TrainingOfficeAdmin` |
| Priority | Must |
| Primary components | `SubjectForm`, `FormActions` |
| Description | Create a single `Subject` reference record. Uppercase code validation; duplicate code shows localized error. Success → toast + redirect `/admin/subjects`. |

**Traceability:** FR-03 · AC-03f · AC-03g
...
| Role | `TrainingOfficeAdmin` |
| Priority | Must |
| Primary components | `ReportFilterBar`, `ReportSummaryCards`, `SessionReportTable`, `Button` (export) |
| Description | Institution-wide attendance reports with same filter pattern as instructor view but unrestricted class scope. |
| Listing compliance | [14-listing §0](./14-listing-pages-search-filter-sort.md) — `ReportFilterBar`, table sort, pagination |
| Export | Inline **Xuất CSV** using active filters ([AC-13d](../brds/08-acceptance-mvp-future.md)) |

**Traceability:** FR-12, FR-13 · AC-12, AC-13
...
| Description | Dedicated confirm/audit export page: configure filters, confirm, download CSV. Complements inline **Xuất CSV** on `/admin/reports` — not the only export entry point. Compliance footer on export actions. |
...
| 2a | Shell overview | `/` | Public (unauth) | `RootLayout` | Must |
...
| 12a | User CSV Import | `/admin/users/import` | Admin | `AdminLayout` | Must |
...
**Total MVP pages:** 22 (19 Must, 2 utility, 1 Should policy page). With nested user edit routes counted separately: **23** routable screens.
...
| --- | --- | --- |
| Route discovery panel | `/login`, `/check-in`, `/sessions`, `/admin` | Unauthenticated visitor on `/` taps quick link ([AC-18f](../brds/08-acceptance-mvp-future.md)) |
...
| **Nhập CSV người dùng** | `/admin/users/import` | User list header |
...
| Student profile / settings page (`/profile`, `/settings`) | Not required for check-in MVP — UserMenu read-only identity panel **is** MVP ([05-common-ui-components.md](./05-common-ui-components.md) §6.3, [AC-02e](../brds/08-acceptance-mvp-future.md)) |
