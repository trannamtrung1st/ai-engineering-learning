...
- Producer batch boundaries and reviewer `review respond` boundaries abort the in-flight turn, wait for `wait_turn_settled`, then close the turn (producer queues the next turn on the same session; reviewer releases the bounded session). Pair `CursorProvider.abort_turn()` with `wait_turn_settled()` when callers must block until the collector finishes; `terminate_session()` does both. For collector-thread races, test with `CursorProvider(..., runner=fake_runner)` — not only `StubProvider`.
